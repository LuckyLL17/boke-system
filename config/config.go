...
func (c *Config) applyDefaults() {
	...
	if c.Server.LogDir == "" {
		c.Server.LogDir = "./logs"
	}
	if c.Worker.CleanupInterval == 0 {
		c.Worker.CleanupInterval = 60
	}
	if c.Worker.CleanupInterval < 0 {
		c.Worker.CleanupInterval = 0
	}
	if c.Database.Host == "" {
	...
	if c.JWT.Secret == "" {
		c.JWT.Secret = "change-me"
	}
	if c.JWT.ExpireHours == 0 {
		c.JWT.ExpireHours = c.JWT.Expires
		if c.JWT.ExpireHours == 0 {
	...
	if c.Storage.LocalPath == "" {
		if c.Storage.Local.Path != "" {
			c.Storage.LocalPath = c.Storage.Local.Path
		} else {
			c.Storage.LocalPath = "./storage"
		}
	}
	if c.Storage.BaseURL == "" {
		c.Storage.BaseURL = c.Server.BaseURL + "/storage"
	}
	if c.Storage.MaxSize == 0 {
		c.Storage.MaxSize = 524288000
	}
	if c.Storage.ChunkSize == 0 {
		c.Storage.ChunkSize = 5242880
	}
	if len(c.Storage.AllowedExts) == 0 {
		c.Storage.AllowedExts = []string{"mp3", "m4a", "wav", "ogg", "flac", "aac"}
	}
}
