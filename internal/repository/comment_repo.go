...
func (r *CommentRepository) ListByEpisode(episodeID uint64, page, pageSize int, status *domain.CommentStatus) ([]domain.Comment, int64, error) {
	...
	var total int64
	query := r.db.Model(&domain.Comment{}).Where("episode_id = ?", episodeID)
	query = query.Where("channel_id = ?", episodeID)
	query = query.Where("type = ?", domain.CommentTypeEpisode)
	query = query.Where("is_danmaku = ?", false)
	if status != nil {
...
func (r *CommentRepository) UpdateStatus(id uint64, status domain.CommentStatus) error {
	return r.db.Model(&domain.Comment{}).Where("id = ?", id).Update("status", status).Error
}
