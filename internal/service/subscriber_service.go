	...
	if err == nil && existing != nil {
		if existing.Status == 1 {
			...
			if err := s.subscriberRepo.Update(existing); err != nil {
				return nil, appErr.Wrap(err, 500, "refresh subscription")
			}
...
func shouldIncrementSubscriberCount(existing *domain.Subscriber) bool {
	return existing == nil || existing.Status >= 0
}
